...
import logo from "../assets/logos.png"; // Import the logo
...
const Users = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        setUsers(querySnapshot.docs.map((doc) => ({ ...doc.data(), id: doc.id }))); // Include the doc ID
      } catch (error) {
  ...
  const handleDelete = async (userId) => {
    try {
      await deleteDoc(doc(db, "users", userId)); // Use the correct document ID
